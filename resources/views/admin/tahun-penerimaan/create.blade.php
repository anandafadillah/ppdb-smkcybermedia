...
@section('title', 'Tambah Tahun Penerimaan')

@section('content')
    <div class="mb-6">
        <h2 class="text-xl font-semibold text-gray-800 dark:text-white/90">Tambah Tahun Penerimaan</h2>
    </div>

    <div class="max-w-lg rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900 shadow-theme-sm p-6">
        <form method="POST" action="{{ route('admin.tahun-penerimaan.store') }}" class="space-y-4">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="tahun" value="{{ old('tahun') }}" placeholder="2025/2026"
            ...
            <div>
                ...
                <input type="text" name="label" value="{{ old('label') }}" placeholder="TA 2025/2026"
                    class="h-11 w-full rounded-lg border bg-transparent px-4 py-2.5 text-sm text-gray-800 dark:text-white/90 dark:bg-gray-900 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden
